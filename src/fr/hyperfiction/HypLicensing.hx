	...
	// -------o public
		...
		public function allow( res : Int ) : Void {
			...
			if( res != RETRY )
				...
				if( _i_retry_count++ < 3 )
					...
					fr.hyperfiction.HypSystem.dialog( "Error" ,  'Cannot connect to the Google Android Licensing server. Please try again later.');
					emit( false );
				}
			}
		}
		...
		*/
		public function dontAllow( res : Int ) : Void {
			trace('dontAllow ::: '+res);
			fr.hyperfiction.HypSystem.dialog( "Error" ,  "Your google account is not authorized to use that app. (error : "+res+")" );
			emit( false );
		}
		...
		*/
		public function applicationError( errorCode : Int ) : Void {
			...
			fr.hyperfiction.HypSystem.dialog( "Error" ,  "An error occured when validating your google account. ( error : "+errorCode+" )" );
			emit( false );
		}
